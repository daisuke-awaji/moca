    ...
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-600 mb-2">
          ...
          <span className="text-gray-400 dark:text-gray-500 ml-1 text-xs">({t('triggers.form.optional')})</span>
        </label>
        <div ref={modelDropdownRef} className="relative">
          <button
            ...
            disabled={disabled}
            className="w-full flex items-center justify-between gap-2 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              {selectedModel ? (
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-gray-400 dark:text-gray-500 text-xs truncate">({selectedModel.provider})</span>
                </div>
              ) : (
                <span className="text-gray-400 dark:text-gray-500">{t('triggers.form.selectModel')}</span>
              )}
            </div>
            <ChevronDown
          ...
          {isModelSelectorOpen && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-50 max-h-64 overflow-y-auto">
              <button
                type="button"
                onClick={handleClearModel}
                className="w-full px-3 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-800 transition-colors text-gray-500 dark:text-gray-400 dark:text-gray-500"
              >
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{model.name}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5 truncate">{model.provider}</div>
                    </div>
                    {model.id === modelId && (
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">{t('triggers.form.modelIdHint')}</p>
      </div>
      ...
      <div>
        ...
        <div ref={directoryDropdownRef} className="relative">
          ...
          >
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <FolderCog className="w-4 h-4 text-gray-400 dark:text-gray-500 flex-shrink-0" />
              {workingDirectory ? (
                <span className="font-mono text-xs truncate">{workingDirectory}</span>
              ) : (
                <span className="text-gray-400 dark:text-gray-500">{t('triggers.form.selectDirectory')}</span>
              )}
            </div>
            <ChevronDown
              className={`w-4 h-4 text-gray-400 transition-transform flex-shrink-0 ${
          ...
          {isDirectorySelectorOpen && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-50 max-h-64 overflow-y-auto">
              ...
              {folders.map((folder) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-gray-900 dark:text-gray-100 truncate">{folder.path}</span>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">{t('triggers.form.workingDirectoryHint')}</p>
      </div>
    </div>
  );
}

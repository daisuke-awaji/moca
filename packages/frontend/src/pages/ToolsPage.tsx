  ...
  const renderParameters = () => {
    ...
    return (
      <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
        <h4 className="text-xs font-medium text-gray-700 dark:text-gray-300 dark:text-gray-600 mb-2">{t('tool.parameters')}:</h4>
        <div className="space-y-2">
          {Object.entries(properties).map(([paramName, paramInfo]) => {
            ...
            return (
              <div key={paramName} className="bg-gray-50 dark:bg-gray-800 px-3 py-2 rounded text-xs">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-blue-600">{paramName}</span>
                  <span className="text-gray-500 dark:text-gray-400 dark:text-gray-500">({typeString})</span>
                  {required.includes(paramName) && (
                    <span className="text-red-500 text-xs">*{t('tool.required')}</span>
                  )}
                </div>
                {description && <p className="text-gray-600 dark:text-gray-400 dark:text-gray-500 mt-1">{description}</p>}
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
      <div className="flex items-start gap-3">
        {/* アイコン */}
        <div className="flex-shrink-0 w-8 h-8 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center text-gray-600 dark:text-gray-400 dark:text-gray-500">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{tool.name}</h3>

            {/* 詳細展開ボタン */}
            {tool.inputSchema.properties && Object.keys(tool.inputSchema.properties).length > 0 && (
              <button
                onClick={() => setIsExpanded(!isExpanded)}
                className="flex-shrink-0 p-1 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-400 rounded"
          ...
          {/* Description */}
          {tool.description && (
            <p className="text-gray-600 dark:text-gray-400 dark:text-gray-500 text-sm mt-1 leading-relaxed">{tool.description}</p>
          )}
      ...
      <PageHeader
        ...
        actions={
          <button
            ...
            className="p-2 text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-700 rounded-lg transition-colors disabled:opacity-50"
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-medium text-gray-900 dark:text-gray-100">{t('tool.searchTools')}</h2>
          </div>
          ...
          {searchQuery && (
            <div className="mt-3 flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 dark:text-gray-500">
              <span>
        ...
        <div>
          ...
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">
              ...
              <span className="ml-2 text-gray-500 dark:text-gray-400 dark:text-gray-500 font-normal text-sm">
                (
                {currentLoading
                  ? t('common.loading')
          ...
          {/* Description文 */}
          {!searchQuery && (
            <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-4">{t('tool.toolsPageDescription')}</p>
          )}
